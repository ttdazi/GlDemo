...
public final class GlUtil {
    ...
    public static final class Attribute {
        ...
        public void bind(int bufferHandle) {
            buffer.position(0);

            GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, bufferHandle);//绑定buffer
            checkGlError();
            ...
            //使用VBO和IBO进行绘制
            GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, bufferHandle); //绑定之前创建好的VBO
    ...
    public static final class Uniform {
        ...
        public void setFloatMat4(float[] value) {
            System.arraycopy(value, 0, this.value, 0, Math.min(this.value.length, value.length));
        }
        ...
        public void setFloatVec4(float v1, float v2, float v3, float v4) {
            this.value[0] = v1;
            this.value[1] = v2;
            this.value[2] = v3;
            this.value[3] = v4;
        }
        public void setFloatMAT4(float[] textureTransform) {
            for (int i = 0; i < textureTransform.length; i++) {
                this.value[i]=textureTransform[i];
            }
        }
    }
    ...
    public static void checkGlError() {
        ...
        while ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
            Log.e(TAG, "glError " + gluErrorString(error) + ", errono:" + error);
            lastError = error;
        }
        if (lastError != GLES20.GL_NO_ERROR) {
            throw new RuntimeException("glError " + gluErrorString(lastError) + ", errno:" + lastError);
        }
    }
    ...
    public static ShortBuffer createBuffer(short[] data) {
        ByteBuffer byteBuffer = ByteBuffer.allocateDirect(data.length * BYTES_PER_SHORT);
    ...
    public static FloatBuffer createBuffer(int capacity) {
        ByteBuffer byteBuffer = ByteBuffer.allocateDirect(capacity * BYTES_PER_FLOAT);
